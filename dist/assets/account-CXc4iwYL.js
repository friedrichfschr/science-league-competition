import{i as e,o as t,t as n}from"./shared-toaPw1j9.js";import{t as r}from"./api-CAqj1E_R.js";var i=`fcm-auth`;function a(){try{return JSON.parse(localStorage.getItem(i))?.token??null}catch{return null}}function o(){try{return JSON.parse(localStorage.getItem(i))?.user??null}catch{return null}}function s(e,t){localStorage.setItem(i,JSON.stringify({token:e,user:t}))}function c(){localStorage.removeItem(i)}async function l(e,t={}){let n=a(),i=await fetch(`${r}${e}`,{...t,headers:{"Content-Type":`application/json`,...n?{Authorization:`Bearer ${n}`}:{},...t.headers}});if(i.status===204)return null;let o=await i.json().catch(()=>({}));if(!i.ok)throw Error(o.message||`HTTP ${i.status}`);return o}function u(e){return e===`administrator`?`<span class="inline-flex items-center rounded-full bg-stone-950 px-3 py-1 text-xs font-bold uppercase tracking-widest text-white">Administrator</span>`:e===`moderator`?`<span class="inline-flex items-center rounded-full bg-amber-100 px-3 py-1 text-xs font-bold uppercase tracking-widest text-amber-800 ring-1 ring-inset ring-amber-200">Moderator</span>`:`<span class="inline-flex items-center rounded-full bg-stone-100 px-3 py-1 text-xs font-semibold uppercase tracking-widest text-stone-600">Mitglied</span>`}function d(e){let t=(e.displayName||e.username||`?`).split(` `).map(e=>e[0]).slice(0,2).join(``).toUpperCase();return`<span class="grid h-20 w-20 place-items-center rounded-full ${e.role===`administrator`?`bg-stone-950 text-white`:e.role===`moderator`?`bg-amber-400 text-stone-950`:`bg-emerald-100 text-emerald-800`} text-2xl font-semibold" aria-hidden="true">${t}</span>`}function f(){return`w-full rounded-xl border border-stone-300 bg-stone-50 px-4 py-3 text-sm text-stone-900 transition placeholder:text-stone-400 focus:border-emerald-400 focus:bg-white focus:ring-2 focus:ring-emerald-100`}function p(){return`mb-1.5 block text-xs font-semibold uppercase tracking-wider text-stone-500`}var m={user:o(),tab:`login`,loading:!1,error:``,registered:!1,resetSent:!1},h=document.querySelector(`#app`);function g(){return m.tab===`forgot`?``:`
    ...
    </div>`}function C(){h.innerHTML=e({activePage:`account`,hero:``,content:m.user?S(m.user):x()}),t(h),n(h)}async function w(){if(a())try{let e=await l(`/api/auth/me`);m.user=e.user??e,s(a(),m.user)}catch{c(),m.user=null}C()}h.addEventListener(`click`,async e=>{let t=e.target.closest(`[data-action]`);if(!t)return;let{action:n,value:r}=t.dataset;if(n===`set-tab`&&(m.tab=r,m.error=``,m.registered=!1,m.resetSent=!1,C(),document.getElementById(`f-username`)?.focus()),n===`logout`){try{await l(`/api/auth/logout`,{method:`POST`})}catch{}c(),m.user=null,m.error=``,C()}if(n===`delete-account`){if(!confirm(`Konto wirklich löschen? Alle deine Beiträge und Daten werden dauerhaft entfernt. Dies kann nicht rückgängig gemacht werden.`))return;try{await l(`/api/auth/me`,{method:`DELETE`}),c(),m.user=null,m.error=``,m.tab=`login`,C()}catch(e){m.error=e.message||`Konto konnte nicht gelöscht werden.`,C()}}}),h.addEventListener(`submit`,async e=>{e.preventDefault();let{action:t}=e.target.dataset;if(![`login`,`register`,`forgot`].includes(t))return;let n=new FormData(e.target);m.loading=!0,m.error=``,C();try{if(t===`login`){let e=await l(`/api/auth/login`,{method:`POST`,body:JSON.stringify({email:n.get(`email`).trim(),password:n.get(`password`)})});s(e.token,e.user),m.user=e.user}if(t===`register`){let e=n.get(`password`);if(e!==n.get(`password2`)){m.error=`Die Passwörter stimmen nicht überein.`,m.loading=!1,C();return}await l(`/api/auth/register`,{method:`POST`,body:JSON.stringify({username:n.get(`username`).trim(),email:n.get(`email`).trim(),password:e})}),m.registered=!0}t===`forgot`&&(await l(`/api/auth/forgot-password`,{method:`POST`,body:JSON.stringify({email:n.get(`email`).trim()})}),m.resetSent=!0)}catch(e){if(t===`forgot`){m.resetSent=!0,m.error=``;return}m.error=e.message||`Ein Fehler ist aufgetreten.`}finally{m.loading=!1,C()}}),w();