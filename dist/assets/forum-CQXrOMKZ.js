import{i as e,o as t,t as n}from"./shared-Cf7orpAm.js";import{t as r}from"./api-CAqj1E_R.js";var i=`fcm-auth`;function a(){try{return JSON.parse(localStorage.getItem(i))?.token??null}catch{return null}}function o(){try{return JSON.parse(localStorage.getItem(i))?.user??null}catch{return null}}function s(e,t){localStorage.setItem(i,JSON.stringify({token:e,user:t}))}function c(){localStorage.removeItem(i)}async function l(e,t={}){let n=a(),i=await fetch(`${r}${e}`,{...t,headers:{"Content-Type":`application/json`,...n?{Authorization:`Bearer ${n}`}:{},...t.headers}});if(i.status===204)return null;let o=await i.json().catch(()=>({}));if(!i.ok)throw Error(o.message||`HTTP ${i.status}`);return o}function u(e){return{...e,createdAt:e.createdAt??e.created_at,commentCount:e.commentCount??e.comment_count??0,votes:e.votes??(e.score===void 0?0:Number(e.score)),userVote:e.userVote??e.my_vote??0,authorId:e.authorId??e.author_id,author:e.author??{id:e.author_id,username:e.author_username,displayName:e.author_displayName??e.author_username,role:e.author_role}}}function d(e){return{...e,createdAt:e.createdAt??e.created_at,votes:e.votes??(e.score===void 0?0:Number(e.score)),userVote:e.userVote??e.my_vote??0,author:e.author??{id:e.author_id,username:e.author_username,displayName:e.author_displayName??e.author_username,role:e.author_role}}}var f={view:`feed`,user:o(),posts:[],currentPost:null,loading:!1,error:null,showNewPost:!1,sortBy:`new`},p=document.querySelector(`#app`);function m(e){let t=(Date.now()-new Date(e))/1e3;return t<60?`Gerade eben`:t<3600?`${Math.floor(t/60)}m`:t<86400?`${Math.floor(t/3600)}h`:`${Math.floor(t/86400)}d`}function h(e){return e===`administrator`?`<span class="inline-flex items-center rounded-full bg-stone-950 px-2 py-0.5 text-[0.6rem] font-bold uppercase tracking-widest text-white">Admin</span>`:e===`moderator`?`<span class="inline-flex items-center rounded-full bg-amber-100 px-2 py-0.5 text-[0.6rem] font-bold uppercase tracking-widest text-amber-800 ring-1 ring-inset ring-amber-200">Mod</span>`:``}function g(e,t=`h-8 w-8 text-xs`){if(!e)return`<span class="grid ${t} shrink-0 place-items-center rounded-full bg-stone-200 text-stone-500 font-semibold">?</span>`;let n=(e.displayName||e.username||`?`).split(` `).map(e=>e[0]).slice(0,2).join(``).toUpperCase();return`<span class="grid ${t} shrink-0 place-items-center rounded-full ${e.role===`administrator`?`bg-stone-950 text-white`:e.role===`moderator`?`bg-amber-400 text-stone-950`:`bg-emerald-100 text-emerald-800`} font-semibold" aria-hidden="true">${n}</span>`}function _(e,t,n,r){let i=!!f.user,a=n===1,o=n===-1;return`
    ...
    </article>`}function x(e,t){let n=e.author,r=e.authorId??e.author_id,i=f.user&&(f.user.id===r||f.user.role===`administrator`||f.user.role===`moderator`),a=t&&r&&r===t;return`
