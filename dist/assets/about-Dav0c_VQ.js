import{a as e,c as t,f as n,i as r,o as i,p as a,r as o,s,t as c}from"./shared-CwnMZZ-T.js";var l=document.querySelector(`#app`);l.innerHTML=r({activePage:`about`,hero:o({eyebrow:s.season,title:`FoodConnectMarkt: Projekt, Angebot und Nutzung klar erklärt.`,intro:`Der FoodConnectMarkt verbindet Einkauf, soziale Angebote und Science-League-Projektidee auf drei klar getrennten Seiten.`,stats:t,supportingCard:`
    ...
    <div class="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.86fr_1.14fr] lg:items-start">
      ...
      <div class="overflow-hidden rounded-[1.75rem] border border-stone-200 bg-white/80">
        <div class="bg-stone-950 px-6 py-6 text-stone-50 sm:px-7">
          ...
          <h3 class="mt-3 text-2xl font-semibold">${i[0].title}</h3>
          <p class="mt-2 text-sm leading-7 text-stone-200">${i[0].text}</p>
        </div>
        <div class="grid divide-y divide-stone-200 md:grid-cols-2 md:divide-x md:divide-y-0">
          ${i.slice(1).map((e,t)=>`
                <article class="px-6 py-5 ${t===0?`bg-stone-50/70`:`bg-white`}">
                  <p class="text-xs font-semibold uppercase tracking-[0.28em] text-emerald-800">Schwerpunkt ${t+2}</p>
                  <h3 class="mt-3 text-2xl font-semibold text-stone-950">${e.title}</h3>
